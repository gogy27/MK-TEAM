...
class TeacherPresenter extends BasePresenter {
	...
	private function getResults($from, $studentID) {
		$results = array();
		$this->studentTasks = $this->unitConversion->getUserTasks($studentID)->order(Model\UnitConversion::UNIT_COLUMN_ID . " DESC")->limit(20, $from);
		foreach ($this->studentTasks as $task) {
	 ...
	 */
	public function handleGetNextResults($studentID, $count) {
		$this->template->tasks = $this->getResults(intval($count), intval($studentID));
